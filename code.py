...
def load_data():
    ...
    try:
        ...
        st.error("CSV file not found. Please ensure 'Fifa 23 Players Data.csv' is in your GitHub repository.")
        return pd.DataFrame()

fifa_data = load_data()

if not fifa_data.empty:
    features = [
    ...
    if menu == "Filter Players":
        ...
        # Safe selection of columns to avoid KeyError
    ...
    # -----------------------------
        ...
        with col2:
            p2 = st.selectbox("Select Player 2", fifa_data["Known As"].unique(), index=min(1, len(fifa_data)-1))
